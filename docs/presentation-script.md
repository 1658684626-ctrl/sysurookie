...
大家好，我们的课程作业题目是赛事运营数字化解决方案，产品叫 EasyEvent 易赛通。
...
大家好，我们的课程作业题目是赛事运营数字化解决方案。我们设计的产品叫 EasyEvent 易赛通，是一个面向中小型体育赛事的一站式数字化管理系统。
...
1. 进入“演示模式”，点击“一键生成完整演示案例”。
2. 打开“项目概览”，说明赛事模板配置。
3. 打开“参赛者端”，选择报名记录，展示报名状态、签到码、点位打卡和通知公告。
4. 打开“管理端”，依次展示报名审核、签到检录、现场执行、通知公告。
5. 打开“数据复盘”，展示核心指标、漏斗、项目表现、点位复盘、通知确认和运营洞察。

## 答辩可能问题

- 为什么不接后端？
  当前是课程 MVP，重点验证核心流程闭环；真实上线可扩展云数据库、文件存储和权限系统。
- 为什么不是普通报名表？
  EasyEvent 同时覆盖赛前报名审核、赛中签到点位、通知确认和赛后复盘。
- 为什么可以复用到其他赛事？
  赛事名称、报名模式、项目路线、成员字段、材料规则、点位和通知类型都来自 EventConfig。
